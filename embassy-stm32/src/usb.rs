...
impl EndpointBuffer {
    fn read(&mut self, buf: &mut [u8]) {
        ...
        for i in 0..((buf.len() + 1) / 2) {
            let val = unsafe { ep_mem_reg(self.addr as usize / 2 + i).read_volatile() };
            buf[i * 2] = val as u8;
            if i * 2 + 1 < buf.len() {
    ...
    fn write(&mut self, buf: &[u8]) {
        ...
        for i in 0..((buf.len() + 1) / 2) {
            ...
            unsafe { ep_mem_reg(self.addr as usize / 2 + i).write_volatile(val) };
        }
    }
}
...
impl<'d, T: Instance, D> Endpoint<'d, T, D> {
    fn write_data(&mut self, buf: &[u8]) {
        ...
        self.buf.write(buf);
        unsafe { ep_in_len(index).write_volatile(buf.len() as _) };
    }

    fn read_data(&mut self, buf: &mut [u8]) -> Result<usize, EndpointError> {
        let index = self.info.addr.index();
        let rx_len = unsafe { ep_out_len(index).read_volatile() as usize } & 0x3F;
